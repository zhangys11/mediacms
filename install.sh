...
# to generate a random password, use: ADMIN_PASS=`python -c "import secrets;chars = 'abcdefghijklmnopqrstuvwxyz0123456789';print(''.join(secrets.choice(chars) for i in range(10)))"`
ADMIN_PASS = "admin"
echo "from users.models import User; User.objects.create_superuser('admin', 'oo@zju.edu.cn', '$ADMIN_PASS')" | python manage.py shell
